...
static unsigned long
   ...
   __attribute__((regparm(3), stdcall))
#endif
   xfind_pages(unsigned const e_type, Elf32_Phdr const *phdr, int phnum, Elf32_Addr old_hi[2])
{
     size_t sz[2] = {0u, 0u};
     size_t hi[2] = {0u, 0u};
     size_t lo[2] = {~0u, ~0u};
     ...
     if ((ET_EXEC != e_type) && 0x20000000u <= hi[0]) {
          level = (0xfc000000u & old_hi[1]) + umax(0x03ffffffu & old_hi[1], 0x03ffffffu & old_hi[0]);
     ...
     for (j = 0; j <= 1; ++j) {
          ...
          addr[j] = mmap((void *)(level + lo[j]), hi[j], PROT_NONE, MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
     }
