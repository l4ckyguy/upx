...
static void unpackExtent(Extent *const xi, Extent *const xo, f_expand *const f_decompress, f_unfilter *f_unf)
{
     while (xo->size) {
          ...
          if (h.sz_cpr < h.sz_unc) {
               ...
               int const j
                  = (*f_decompress)((unsigned char *)xi->buf, h.sz_cpr, (unsigned char *)xo->buf, &out_len, h.b_method);
...
static Mach_ppc_thread_state const *do_xmap(Mach_header const *const mhdr,
                                            ...
                                            f_unfilter *const f_unf)
{
     ...
     for (j = 0; j < mhdr->ncmds; ++j, (sc = (Mach_segment_command const *)(sc->cmdsize + (char const *)sc)))
...
Mach_ppc_thread_state const *upx_main(struct l_info const *const li,
                                      ...
                                      Mach_header **const mhdrpp)
{
...

     {
          ...
          for (j = 0; j < mhdr->ncmds; ++j, (lc = (Mach_load_command const *)(lc->cmdsize + (char const *)lc)))
