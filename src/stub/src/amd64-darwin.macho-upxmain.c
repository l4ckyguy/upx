...
static void xread(Extent *x, Addr buf, size_t count)
{
     Addr p = x->buf, q = buf;
     size_t j;
...
static void upx_bzero(Addr p, size_t len)
{
     if (len)
          do {
               *p++ = 0;
...
enum e6 { AMD64_THREAD_STATE = 4 };
enum e7 { AMD64_THREAD_STATE_COUNT = sizeof(Mach_AMD64_thread_state) / 4 };
...
static uint64_t do_xmap(Mach_header64 const *const mhdr,
                        ...
                        f_unfilter *const f_unf)
{
     ...
     for (j = 0; j < mhdr->ncmds; ++j, (sc = (Mach_segment_command const *)(sc->cmdsize + (unsigned char const *)sc)))
          if (LC_SEGMENT_64 == sc->cmd && sc->vmsize == 0) {
               ...
               else if (LC_UNIXTHREAD == sc->cmd || LC_THREAD == sc->cmd)
               {
                    ...
                    if (AMD64_THREAD_STATE == thrc->flavor && AMD64_THREAD_STATE_COUNT == thrc->count) {
                         entry = thrc->state.rip + base;
                    }
               }
     ...
     static off_t_upx_stub fat_find(Fat_header * fh)
     {
     ...
     uint64_t upx_main(struct l_info const *const li,
                       size_t volatile sz_compressed,
                       Mach_header64 *const mhdr,
                       size_t const sz_mhdr,
                       f_expand *const f_decompress,
                       f_unfilter *const f_unf,
                       Mach_header64 **const mhdrpp)
     {
          uint64_t entry;
          off_t_upx_stub fat_offset = 0;
          Extent xi, xo, xi0;
          xi.buf = CONST_CAST(Addr, 1 + (struct p_info const *)(1 + li));
          ...
          DPRINTF(
             (STR_upx_main(), sz_compressed, f_decompress, f_unf, &xo, xo.size, xo.buf, &xi, xi.size, xi.buf, mhdrpp));
...

          {
               ...
               for (j = 0; j < mhdr->ncmds;
                    ++j, (lc = (Mach_load_command const *)(lc->cmdsize + (unsigned char const *)lc)))
